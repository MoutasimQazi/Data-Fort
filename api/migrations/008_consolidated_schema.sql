-- Datafort — consolidated tenant schema (MySQL 8 / MariaDB 10.4+)
--
-- THE canonical schema for a fresh tenant database, superseding the
-- incremental sequence 001_schema.sql + 005_reveal_once.sql +
-- 006_daily_assignment.sql + 007_alerts_seen.sql for every NEW install
-- from here on — scripts/provision-tenant.php and
-- api/platform/tenants-save.php's provision_schema action both apply
-- THIS file alone. The four originals are left in place unchanged
-- because Movenetics' own live database was built by running them one
-- at a time and already has every column they added — there is nothing
-- to gain and real risk in touching a live database's history to match
-- this file retroactively.
--
-- Two differences from that incremental sequence, both deliberate:
--
--   1. Every column 005/006/007 added is folded directly into the
--      CREATE TABLE below (leads.assigned_at + its index,
--      users.daily_lead_target, users.alerts_seen_at) — there is
--      nothing left to retrofit on a database that never existed
--      before this file ran.
--
--   2. THERE IS NO SEED BLOCK. 001_schema.sql ends by inserting
--      Movenetics' own tenant row and an admin@moveneticsdigital.com
--      account with a hardcoded password hash — baked into the same
--      file as the table definitions. Applied to any other tenant's
--      database that plants a stray "movenetics" row and a copy of a
--      credential that is sitting in version control, in every new
--      customer's database. scripts/provision-tenant.php used to work
--      around this by string-matching for a "-- ══ Seed ══" marker and
--      truncating the file before running it — fragile, and easy for a
--      future change to 001_schema.sql to silently break. This file
--      has no seed to strip in the first place: seeding a tenant's row
--      and its first admin is the provisioning code's job (random
--      unusable password, one-time invite link — see
--      scripts/provision-tenant.php's seedTenantAndAdmin() or
--      api/platform/tenants-save.php's 'seed_admin' action), never the
--      schema's.
--
...
-- tenant_id. That is weaker than RLS — a missing WHERE clause is a
-- cross-tenant leak rather than an empty result — so it must be
-- covered by review and by the tests in api/README.md.
--
-- In the current architecture each tenant has its OWN database (see
-- api/tenant-resolver.php), so tenant_id here identifies the one row
-- in this database's own `tenants` table — but every query still
-- carries it, both because application code was written that way
-- before that architecture existed, and because it costs nothing and
-- keeps every query safe if a database were ever shared again.
...
CREATE TABLE IF NOT EXISTS users (
  ...
  -- How many NEW leads this rep should receive per day. NOT the same
  -- thing as daily_quota:
  ...
  daily_lead_target SMALLINT UNSIGNED NOT NULL DEFAULT 20,

  status        ENUM('active','flagged','suspended') NOT NULL DEFAULT 'active',
  totp_secret   VARCHAR(64) NULL,          -- optional 2FA (base32)
  last_seen_at  DATETIME NULL,

  -- Lets the "Needs attention" dashboard badge clear once an admin has
  -- actually looked at it, rather than staying red forever.
  alerts_seen_at DATETIME NULL,
...
CREATE TABLE IF NOT EXISTS leads (
  ...
  -- Set every time ownership changes — what makes a "daily batch" a
  -- real, measurable thing rather than a figure of speech.
  ...
  KEY ix_leads_assigned (tenant_id, owner_id, assigned_at),
...
--
-- uq_reveal_once ensures a user can be charged only once for the same
-- lead's same field, ever — without it, refreshing a reveal view could
-- silently burn quota twice for one contact.
...
--   GRANT SELECT, INSERT ON <this database>.audit_log TO '<app user>'@'%';
-- Without that grant, "append-only" is a promise rather than a control.
-- scripts/provision-tenant.php and the platform panel's provision_schema
-- action both apply this grant automatically when they create a
-- tenant's database user.
...
-- No seed block. See the file header — seeding a tenant row and its
-- first admin is provisioning code's job, not this file's.
